...
- エッセイと制作物は既存フィード、書評は公開ブログ本文、日記・音声は既存データから掲載。
...
- public/images/editorial/bookshop.webp
...
TypeScript and lint passed (existing img-element warnings). Desktop 1440px and mobile 390px/320px checked in browser. Hero starts at viewport top; transparent header has no visible border and becomes white below hero. Mobile menu and journal/blog switch tested. Three curated archive URLs returned HTTP 200. Production compilation also passed. Full archive export was not completed during the interactive design edits. The final text edit was checked again with TypeScript.
