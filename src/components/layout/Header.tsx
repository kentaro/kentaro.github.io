...
export default function Header({ activeNav = "home" }: Props) {
	...
	useEffect(() => {
		if (!isHome) return;
		const hero = document.querySelector(".ed-cover");
		if (!hero) return;
		const update = () =>
			setPastHero(
	...
	useEffect(() => {
		...
		const background = Array.from(
			document.querySelectorAll<HTMLElement>(
				".page > header, .page > main, .page > footer, .skip-link",
			),
		);
	...
	return (
		<Fragment>
			<header
				ref={header}
				className={`bar${isHome ? ` bar-home${pastHero ? " bar-solid" : ""}` : ""}`}
			>
				<div className="wrap bar-inner">
					<Link
						href="/"
						className="mast"
						aria-label="栗林健太郎 ホーム"
