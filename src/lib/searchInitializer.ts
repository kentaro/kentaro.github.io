...
export function addCompletionListener(listener: () => void) {
  ...
  if (!alreadyRegistered) {
    console.log("[SearchInitializer] Adding completion listener, count before:", completionListeners.length);
    completionListeners.push(listener);
    console.log("[SearchInitializer] New listener count:", completionListeners.length);
  } else {
  ...
  if (state.isInitialized) {
    console.log("[SearchInitializer] DB already initialized, calling listener immediately");
    try {
      // コールスタックをクリアするために非同期実行
      setTimeout(() => {
        if (state.isInitialized) { // 念のため二重チェック
          listener();
        }
      }, 0);
    } catch (error) {
      console.error('[SearchInitializer] Error calling completion listener:', error);
    }
  }
  
  // クリーンアップ関数を返す
  return () => {
    console.log("[SearchInitializer] Removing completion listener, count before:", completionListeners.length);
    const index = completionListeners.indexOf(listener);
    if (index !== -1) {
      completionListeners.splice(index, 1);
      console.log("[SearchInitializer] New listener count after removal:", completionListeners.length);
    } else {
...
function notifyCompletionListeners() {
  ...
  console.log(`[SearchInitializer] Notifying ${listenerCount} completion listeners (first time)`);
 ...
 * コールバックを登録する関数
 */
export function registerProgressCallback(id: string, callback: (progress: LoadingProgress) => void) {
  ...
  if (state.isInitialized) {
    console.log("[SearchInitializer] DB already initialized, sending completion status");
    ...
    // 初期化中なら現在の進捗を通知
    console.log("[SearchInitializer] Currently initializing, sending current progress");
    ...
    // 初期化前なら初期状態を通知
    console.log("[SearchInitializer] Not yet initialized, sending initial state");
    try {
...
function notifyProgressCallbacks(progress: LoadingProgress) {
  ...
  Object.values(progressCallbacks).forEach(callback => {
    try {
      // 参照切りで新しいオブジェクトとして渡す
      callback({ ...progress });
    } catch (error) {
...
export async function initSearch() {
  // 既に初期化済みの場合は早期リターン
  if (state.isInitialized) {
    console.log("[SearchInitializer] Search already initialized, skipping initialization");
    ...
    // 完了リスナーにも通知（初回のみ）
    if (!hasNotifiedCompletion) {
      console.log("[SearchInitializer] First call to initialized DB, notifying completion listeners");
      notifyCompletionListeners();
    } else {
      console.log("[SearchInitializer] Already notified completion, skipping redundant notification");
    }
    return;
  }
  ...
  try {
    ...
    // 完了通知を即時実行
    if (!hasNotifiedCompletion) {
      console.log("[SearchInitializer] Calling notifyCompletionListeners immediately after initialization");
      notifyCompletionListeners();
    } else {
      console.log("[SearchInitializer] Completion already notified, not notifying again");
    }
    
  } catch (error) {
    console.error("[SearchInitializer] Error initializing search system:", error);
    ...
    notifyProgressCallbacks({
      ...
      progress: 0,
      total: 100,
      error: error instanceof Error ? error.message : "検索システムの初期化中にエラーが発生しました"
    });
